...
namespace DNA.Negocios.Cadastral.WEB
{
    public class RastreamentoPJPrata
    {
        ...
        public Entidades.Cadastral.ResponsePJPrata PesquisaPJPrata(string numeroCNPJ)
        {
            try
            {
                ...
                neg.PesquisaPJ(numeroCNPJ, ref ds);
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    foreach (DataRow dr in ds.Tables[4].Rows)
                    {
                        ...
                        qsa.TipoPessoa = dr["TIPO_PESSOA"].ToString();

                        //if (!qsa.DocumentoSocio.Trim().Equals(""))
                        //{ qsa.TipoPessoa = qsa.DocumentoSocio.Trim().Length > 11 ? "PJ" : "PF"; }

                        if (!qsa.DataEntradaSociedade.Trim().Equals(""))
                        {
                            try
                            { qsa.DataEntradaSociedade = DateTime.Parse(qsa.DataEntradaSociedade).ToString("dd/MM/yyyy"); }
                            catch 
                            { qsa.DataEntradaSociedade = ""; }
                        }
