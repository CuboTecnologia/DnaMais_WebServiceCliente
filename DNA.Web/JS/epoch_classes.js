...
function Epoch(name,mode,targetelement,multiselect) 
{
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
	{
		...
		this.calendar.style.zIndex = 4000;
...
{
    // Incluido por Jairo em 15/08/2015 para pegar o ano corrente
    var dataCorrente = new Date().getFullYear();
    
    //this.mode = 'flat'; //can be 'flat' or 'popup'
    this.displayYearInitial = this.curDate.getFullYear(); //the initial year to display on load
    this.displayMonthInitial = this.curDate.getMonth(); //the initial month to display on load (0-11)
    this.rangeYearLower = 1850;
    this.rangeYearUpper = dataCorrente;
    this.minDate = new Date(1850, 0, 1);
    this.maxDate = new Date(dataCorrente, 0, 1);
...
{
	this.daylist = new Array('Dom','Seg','Ter','Qua','Qui','Sex','Sab','Dom','Seg','Ter','Qua','Qui','Sex','Sab'); /*<lang:en>*/
	...
	this.clearbtn_title = 'Limpa as datas selecionadas';
...
/*
    Alterado: 26/10/2010
    Verifica Versao do Browser.
    Objetivo Sobrepor o Calendario a todos os selects da Página
...
var selectTop = {
    Collection: function()
    {
        ...
        if (myBrowser.Version() == 6)
        {
            var selects = document.getElementsByTagName('select');
            
            for (i = 0; i < selects.length; i++)
            {
                iTop += Epoch.prototype.getTop(selects[i]);
                collection += (collection != '' ? '|' : '') + selects[i].id + '@' + iTop + '\n';
            }   
        }
...
var selectLeft = {
    Collection: function()
    {
        ...
        if (myBrowser.Version() == 6)
        {
            ...
            for (i = 0; i < selects.length; i++)
            {
                iTop += Epoch.prototype.getLeft(selects[i]);
                collection += (collection != '' ? '|' : '') + iTop;
            }   
        }
...
/*
    Alterado: 26/10/2010
    Inclusão da verificação se oNode != null. Ajuste necessário p/ MasterPage
    Por: Fernando Domingues
 */
...
/*
    Alterado: 26/10/2010
    Inclusão da verificação se oNode != nulo. Ajuste necessário p/ MasterPage
    Por: Fernando Domingues
 */
...
Epoch.prototype.show = function () //PUBLIC: displays the calendar
{
	...
	if(myBrowser.Version() == 6)
	{
        var posX = this.calendar.style.top;
        var posY = this.calendar.style.left;
        var width = this.calendar.style.clientWidth;   
        var height = this.calendar.style.clientHeight; 
        ...
        for (i = 0; i < selects.length; i++)
        {
            var selX = Epoch.prototype.getTop(selects[i]);
            var selY = Epoch.prototype.getLeft(selects[i]);
            ...
            if(posX<selX) 
            {   
                //if(posX+width>selX) { onX = true; }   
                onX = true;
            } 
            else 
            {   
                if((selX+widthS)>(posX+width)){onX = true;}   
            }   
            ...
            if(posY > selY) 
            {   
               // if((posY+height) > selY){onY = true; }
               onY = true;  
            } 
            else 
            {   
                if((selY+heightS) > (posY+height) ){ onY = true; }  
            }   
...
function trickSelect(obj,visibility)
{
        var posX = obj.screenTop;
        var posY = obj.style.left;
...
Epoch.prototype.createCalCells = function ()  //PRIVATE: creates the table containing the calendar day cells
{
	...
	for(var i=0;i<totalCells;i++)
	{
		...
		// Alterado por Jairo Simoes em 23/10/2010
		// Correção de Bug
		// Por causa do horario de verao, o incrementador que conta apartir da meia noite [metodo Date(beginDate)]
		// acaba repetindo o dia no calendário, bagunçando todas as datas.
		// Inclui um validados que ve se a data foi incrementada ou não. Caso NÃO
		// tenha sido incremetada, a rotina 'força' a incrementação.
		sdt.setHours(2);
